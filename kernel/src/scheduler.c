...
void scheduler_init(void) {
  ...
  for (size_t i = 0; i < g_cpu_state_table_len; i++) {
    ...
    asserts(llrb_tdeadline_new(&s->deadlines),
            "scheduler: deadline tree alloc failed");
...
[[noreturn]] void scheduler_loop(struct scheduler *scheduler) {
  while (true) {
    ...
    if (next == nullptr) {
      ...
      // No thread means no scheduling quantum, but userspace timers owned by
      // this CPU must remain armed while it is idle.
      timer_cpu_idle();
      ...
      timer_cpu_dispatch(SCHED_QUANTUM_US * 1000ull);
